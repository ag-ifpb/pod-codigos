...
public class MarshalerImpl implements Marshaller{
	private byte first = 0x0;//|0|0|0|0|0|0|0|0|
	private byte second = 0x0;
	private byte[] message = new byte[1024];

	@Override
	public Marshaller length(int value) {
		//
		if (value > 1024) throw new RuntimeException("O tamanho da mensagem "
				+ "neste frame não pode ser superior a 1024");
		//
		int multp;
		int left = (value % 256);
		//
	...
	public Marshaller message(String value) {
		//
		if (value.length() > 1024) throw new RuntimeException("O tamanho da mensagem "
				+ "neste frame não pode ser superior a 1024");
